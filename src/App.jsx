...
function App() {
  ...
  function handleChangeStatus(day, newStatus, id){
    ...
    setWeekData(prevWeekData => {
      let editedEntry = [prevWeekData[day.toLowerCase()].filter(task => task.id === id)];
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center">
        {DAYS.map(day => <DayColumn key={day} tasks={weekData[day]} onNewTask={handleAddTask} onChangeStatus={handleChangeStatus}>{day.toUpperCase()}</DayColumn>)}
      </div>
    </>
  );
}
